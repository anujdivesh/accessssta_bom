...
#/opendap/allData/ghrsst/data/GDS2/L4/GLOB/JPL/MUR/v4.1/2017/273/20170930090000-JPL-L4_GHRSST-SSTfnd-MUR-GLOB-v02.0-fv04.1.nc

def Download_MUR(file_path, data_dir, data_subdir, day, month, year, reg_name):
    mur_path=data_dir+'/'+data_subdir
    gridcell = 360/float(36000)#cell size 0.01degree
...
#    lonRange = range(lonCellStart, lonCellEnd)
    ...
    lat,lon,sst_data,sst_time=extract_sst(file_path,lonRange,latRange,variable_name)
...
def extract_sst(filename,lonRange,latRange,nc_data_name):
...
#    lons = cdf1.variables["lon"][:]
#    zeroIndex = np.where(lons == 0.)[0][0]
#    rolls = lons.size - zeroIndex
#    shiftlons = np.roll(lons, rolls)
#    lon[:]=shiftlons[lonRange]
#    lon = map(lambda x: x+ 360.0 if x < 0.0 else x, lon)
...
#	for i in range(len(lon_array)):
#		if lon_array[i]<0:
#			lon_array[i]=lon_array[i]+180
#        lon_array = lon_array + 180
