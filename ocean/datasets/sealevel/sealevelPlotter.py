...
def plotTimeseries(outputFilename, saveData=True, **args):
    ...
    titlePrefix = {
        'alt': "Altimetry",
        'rec': "Reconstruction"}[variable]

    grid_data = SeaLevelSeries(variable,
                          latrange=(lat,lat),
                          lonrange=(lon,lon))
    ...
    if saveData:
        with open(outputFilename + ".txt", 'w') as file:
            ...
            for date, height in zip(grid_data.time, grid_data.data):
    ...
    ax.grid(True)
    ax.set_ylim(-350, 350)
    ax.plot(grid_data.time, grid_data.data, 'b-')
    plt.axhline(y=0, color='k')
