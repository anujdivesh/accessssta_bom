...
class CoralPlotterWrapper(SurfacePlotter):
    ...
    VARIABLE_MAP = {
        'daily': 'bleaching_alert_area',
        'outlook': 'CRW_BAA_Week_',
    }
...
#        return 'outlook_srt_v3_wk_among28_cfsv2_icwk'
    ...
    @apply_to(variable='daily')
    def get_prefix(self, params={}):
        return 'ct5km_baa-max-7d_v3.1_'

    @apply_to(variable='outlook')
    def get_prefix(self, params={}):
        return 'cfsv2_outlook-060perc_4mon-and-wkly_v5_icwk'


#    def get_prefix(self, params={}):
#        prefix = ''
#        if params['variable'] == 'daily':
#            prefix = 'b5km_baa-max-7d_'
#        else:
#            prefix = 'cfsv2_outlook-060perc_4mon-and-wkly_v4_icwk'
#        return prefix 

    def get_formatted_date(self, params={}):
        formatted_date = ''
        #path = getLastestFile(self.get_path(params = params))
        fileName = os.path.join(self.get_path(params = params), '*.nc')
        ...
        latestFilePath = sorted(glob.iglob(fileName), cmp=cmp, key=lambda x: x[date_index_start:date_index_end])
        path=latestFilePath[-1]
         
        refDate = datetime.strptime(path[date_index_start:date_index_end], '%Y%m%d')         
        ...
        if params['period'] == '4weeks':
            ...
            incDate = startDate + timedelta(weeks=3)
            formatted_date = incDate.strftime('%d %B %Y')
        elif params['period'] == '8weeks':
            incDate = startDate + timedelta(weeks=7)
            formatted_date = incDate.strftime('%d %B %Y')
        elif params['period'] == '12weeks':
            incDate = startDate + timedelta(weeks=11)
            formatted_date = incDate.strftime('%d %B %Y')
...
#    @apply_to(variable='daily')
	...
	return CoralGridset(self.get_path(params=params), gridvar, params['period'],
...
class CoralGridset(Gridset):
    ...
    def get_filename(self, path, prefix, suffix, date, period):
        ...
        latestFilePath =sorted(glob.iglob(fileName), cmp=cmp)
