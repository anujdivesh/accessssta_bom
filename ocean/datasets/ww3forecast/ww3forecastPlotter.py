...
from ocean.plotter import Plotter, COMMON_FILES,EXTRA_FILES, getCopyright, get_tick_values, discrete_cmap
...
class Ww3ForecastPlotter(Plotter):
    ...
    def get_ticks(self, params={}):
        return np.array(customTicks[variableConfig[params['variable']][1]['cb_ticks']])
...
#        return False
    ...
    def get_colors(self, params={}):
        return customColorMap[variableConfig[params['variable']][1]['cm']] 
    ...
    def plot_basemaps_and_colorbar(self, *args, **kwargs):
        ...
        fill_color = kwargs.get('fill_color', '0.02')
...
#        d_cmap = mpl.colors.ListedColormap(np.array(cmArray) / 255.0)
        d_cmap, norm = from_levels_and_colors(cm_edge_values, np.array(cmArray) / 255.0)
        basemap_cmap, basemap_norm = from_levels_and_colors(cm_edge_values, np.array(basemapArray) / 255.0)
        cm_edge_values = np.array(cm_edge_values)
    ...
    def plot_surface_data(self, *args, **kwargs):
	    ...
	    #GAS this was removed because different colour ranges makes comparison difficult
            ...
            if colormap_strategy == 'discrete':
                ...
                d_cmap, norm = from_levels_and_colors(cm_edge_values, None, extend=extend)
            ...
            if contourLines:
                ...
                cnt = plt.contour(x, y, data, levels=cm_edge_values, norm=norm,
                                 colors = 'w', linewidths = 0.4, hold='on')
                if contourLabels:
                    labels = plt.clabel(cnt, cm_edge_values[::4], inline=True, fmt=cb_tick_fmt, colors='k', fontsize=5, zorder=2)
                    bbox_props = dict(boxstyle="round", fc="w", ec="w")
                    for text in labels:
                        text.set_linespacing(1)
                        text.set_bbox(bbox_props)
