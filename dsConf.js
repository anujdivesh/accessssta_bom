    ...
    waveatlas: {
        ...
        onSelect: function(){
            if (ocean.variable == 'atlas'){
                ...
                //Read file
                $.getJSON("config/comp/wacop.json",function(data) {
                          })
                             .done(function(points) {
                               for (var i = 0; i < points.length; i++) {
                                 var marker = new L.marker([points[i][1],points[i][2]])
                                   .bindPopup("<b>Location: "+points[i][0] + "</b><br>" + "<a href=http://wacop.gsd.spc.int/" + points[i][4].trim() + " target=_blank>See wave climate report</a>");
    ...
    tideforecast: {
        ...
        onSelect: function(){
            ...
            if (ocean.variable == 'tide'){
                ...
                //Read file
                //a.appUrl="config/comp/app.json";a.getData=function(){return $.when($.getJSON(a.appUrl)).then(function(b){
                $.getJSON("config/comp/tideforecast.json",function(data) {
                          })
                             .done(function(points) {
                               for (var i = 0; i < points.length; i++) {
...
function getTideInfo(e) {
    ...
    .done(function(text) {
      ...
      if (data.next_high.time && (data.next_high.time - data.current_time > 0)){
      ...
      if (data.next_low.time && (data.next_low.time - data.current_time > 0)){
    ...
    .fail(function(jqxhr, textStatus, error) {
      fatal_error("Failed to retrieve tide forecast information.");
    });
}
