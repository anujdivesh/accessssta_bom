...
from ocean.datasets.accesss import ACCESSS
from ocean.netcdf import SurfacePlotter, Gridset, Grid
from accessssstPlotter import AccesssSstPlotter
...
accesssProduct = productName.products['accesss']

#number of forecast steps
FORECAST_STEPS = 5

class AccessPlotterWrapper(SurfacePlotter):
    DATASET = 'accesssssta'
    PRODUCT_NAME = "ACCESS-S Forecast"
...
#        return 'cm'
    ...
    def get_grid(self, params={}, **kwargs):
        ...
        grid =  AccesssGridset(self.get_path(params=params), gridvar, params['period'],
...
class AccesssGridset(Gridset):
    ...
    def get_filename(self, path, prefix, suffix, date, period):
        ...
        """
        # mds - 20201206 fileName = os.path.join(path, 'sst.forecast.anom.monthly.nc')
        fileName = os.path.join(path, 'sst.forecast.anom.monthly.nc')
        # mds - 20210822 fileName = os.path.join(path, 'accessssst.nc')
        return  fileName
...
class accesssssta(ACCESSS):
        
    DATASET = 'accesssssta'
    PLOTTER = AccessPlotterWrapper
   ...
   #         self.plotter.plot_basemaps_and_colorbar(self.getPlotFileName(var, step, region)[1], step,  args)
            plot_filename = '%s_%s_%s_%02d' % (accesssProduct[var], var, region, step)
            args['step'] = step
            args['area'] = region
    ...
    def generateConfig(self, params):
        ...
        '''
        baseDateTime = datetime(1970, 1, 1, 0, 0, 0)
    ...
    def getPlotFileName(self, varName, timeIndex, regionName):
        ...
        '''
        plot_filename = '%s_%s_%s_%02d' % (accesssProduct[varName], varName, regionName, timeIndex)
